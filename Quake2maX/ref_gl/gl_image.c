...
qboolean picMipmap (image_t	*image)
{
	if (image->type == it_part && image->script)
	{
...
*/
/*
// This function doesn't need to call it as the others, but to the first rendering if r_blurtexture == NULL, otherwise r_refdef is not updated - mysticman
void R_InitBlurTexture( void )
{
   qbyte *data;
       int vwidth = 1, vheight = 1;
   while (vwidth < r_refdef.width)   { vwidth *= 2; }
   while (vheight < r_refdef.height) {   vheight *= 2; }
   data = Mem_TempMalloc( vwidth * vheight * 4 );
   r_blurtexture = R_LoadPic( "***r_blurtexture***", &data, vwidth, vheight, IT_NOPICMIP|IT_NOMIPMAP|IT_NOCOMPRESS, 3 );
   Mem_TempFree( data );
} 
*/
...
void LoadTGA (char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	{ 
		ri.Con_Printf (PRINT_ALL, "LoadTGA: Bad tga file %s\n", name); 
...
nonscrap:*/
		...
		if (bits == 8)
			image->has_alpha = GL_Upload8 (pic, width, height, picMipmap(image), image->type == it_sky );
		else
			image->has_alpha = GL_Upload32 ((unsigned *)pic, width, height, picMipmap(image) );
...
image_t	* checkOtherFormats (char *basename, imagetype_t type)
{
	int i,j;
	image_t	*image = NULL;

	for (i=0;i<IMAGETYPES && !image;i++)
		if (image = GL_FindImage(va("%s%s", basename, image_types[i]), type))
			return image;
...
image_t	*GL_FindImage (char *name, imagetype_t type)
{
	...
	//get rid of suffix...
	Com_sprintf(base_name, sizeof(base_name), "%s", name);
	dot = strstr (base_name, ".");
	if (dot)
	{
		spot = strlen(base_name)-strlen(dot);
		base_name[spot] = 0;
	}
	...
	if (!strcmp(name+len-4, ".pcx"))
	{
		...
		image = checkOtherFormats (&base_name[0], type);
	...
	if (strlen(base_name)>5
		&& (base_name[0]=='P' || base_name[0]=='p')
		&& (base_name[1]=='I' || base_name[1]=='i')
		&& (base_name[2]=='C' || base_name[2]=='c')
		&& (base_name[3]=='S' || base_name[3]=='s')
		&& base_name[4]=='/')
		Com_sprintf(image->bare_name, sizeof(image->bare_name), "%s", &base_name[5]);
	else
		Com_sprintf(image->bare_name, sizeof(image->bare_name), "%s", &base_name[0]);

	if (dot)
		base_name[spot] = (char)135;
...
void GL_FreeUnusedImages (void)
{
	...
	r_celtexture->registration_sequence = registration_sequence;
	r_particlebeam->registration_sequence = registration_sequence;
	r_dynamicimage->registration_sequence = registration_sequence;
