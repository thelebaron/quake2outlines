	...
	for (protocol = 0 ; protocol < 2 ; protocol++)
	{
		...
		if (ret == -1)
		{
			...
			if (err == WSAEWOULDBLOCK)
				continue;
			if (err == WSAEMSGSIZE) {
				Com_Printf ("^1Warning:  Oversize packet from %s\n",
...
void NET_SendPacket (netsrc_t sock, int length, void *data, netadr_t to)
{
	...
	if (ret == -1)
	{
		...
		if (dedicated->value)	// let dedicated servers continue after errors
		{
			Com_Printf ("^2NET_SendPacket ERROR: %s to %s\n", NET_ErrorString(),
				NET_AdrToString (to));
		}
		else
		{
...
int NET_IPSocket (char *net_interface, int port)
{
	...
	if ((newsocket = socket (PF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1)
	{
		...
		if (err != WSAEAFNOSUPPORT)
			Com_Printf ("^1WARNING: UDP_OpenSocket: socket: %s", NET_ErrorString());
		return 0;
	}
	...
	{
		Com_Printf ("^1WARNING: UDP_OpenSocket: ioctl FIONBIO: %s\n", NET_ErrorString());
		return 0;
	}
	...
	{
		Com_Printf ("^1WARNING: UDP_OpenSocket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString());
		return 0;
	}
	...
	{
		Com_Printf ("^1WARNING: UDP_OpenSocket: bind: %s\n", NET_ErrorString());
...
int NET_IPXSocket (int port)
{
	...
	if ((newsocket = socket (PF_IPX, SOCK_DGRAM, NSPROTO_IPX)) == -1)
	{
		err = WSAGetLastError();
		if (err != WSAEAFNOSUPPORT)
			Com_Printf ("^1WARNING: IPX_Socket: socket: %s\n", NET_ErrorString());
		return 0;
	}
	...
	if (ioctlsocket (newsocket, FIONBIO, &_true) == -1)
	{
		Com_Printf ("^1WARNING: IPX_Socket: ioctl FIONBIO: %s\n", NET_ErrorString());
		return 0;
	}
	...
	{
		Com_Printf ("^1WARNING: IPX_Socket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString());
		return 0;
	}
	...
	if( bind (newsocket, (void *)&address, sizeof(address)) == -1)
	{
		Com_Printf ("^1WARNING: IPX_Socket: bind: %s\n", NET_ErrorString());
		closesocket (newsocket);
		return 0;
	}
