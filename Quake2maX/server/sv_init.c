...
int SV_FindIndex (char *name, int start, int max, qboolean create)
{
	...
	{ 
		if (start == CS_MODELS) 
			Com_Printf ("^1Warning: Index overflow for models\n"); 
		else if (start == CS_SOUNDS) 
			Com_Printf ("^1Warning: Index overflow for sounds\n"); 
		else if (start == CS_IMAGES) 
			Com_Printf ("^1Warning: Index overflow for images\n"); 
...
void SV_InitGame (void)
{
	...
	Cvar_FullSet ("sv_engine", GAMENAME, CVAR_SERVERINFO | CVAR_NOSET | CVAR_LATCH);
	Cvar_FullSet ("sv_engine_version", VERSION, CVAR_SERVERINFO | CVAR_NOSET | CVAR_LATCH);
}
