...
void SV_WriteServerFile (qboolean autosave)
{
	FILE	*f;
	cvar_t	*var;
	char	name[MAX_OSPATH], string[MAX_OSPATH];
	char	comment[32];
	time_t	aclock;
	struct tm	*newtime;
...
void ActionStartMod(char*);
void SV_NewMod_f (void)
{
	if (Cmd_Argc() != 2)
	{
		Com_Printf ("USAGE: newmod <modification directory>\n");
		return;
	}

	ActionStartMod(Cmd_Argv(1));
}
...
void SV_InitOperatorCommands (void)
{
	...
	Cmd_AddCommand ("modification", SV_NewMod_f);
