...
byte		cmd_text_buf[32768]; //8192
byte		defer_text_buf[32768]; //8192
...
void Cmd_Exec_f (void)
{
	...
	scriptname = Cmd_Argv(1);

	//AutoFind if now suffix
	if (!strstr (scriptname, ".")) //add .cfg for easy use
	{
		scriptname = va("%s.cfg", Cmd_Argv(1));
		if (FS_LoadFile (scriptname, NULL) == -1)
		{
			scriptname = va("%s.rc", Cmd_Argv(1));
			if (FS_LoadFile (scriptname, NULL) == -1)
			{
				scriptname = va("%s.c", Cmd_Argv(1));
				if (FS_LoadFile (scriptname, NULL) == -1)
				{
	...
	len = FS_LoadFile (scriptname, (void **)&f);
	if (!f)
	{
		Com_Printf ("couldn't exec %s\n",scriptname);
		return;
