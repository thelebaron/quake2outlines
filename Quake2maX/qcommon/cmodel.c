					  ...
					  trace_t *trace, cbrush_t *brush)
{
	...
	if (enterfrac < leavefrac)
	{
		if (enterfrac > -1 && enterfrac < trace->fraction)
		{
			...
			trace->plane.normal[0] = clipplane->normal[0];
			trace->plane.normal[1] = clipplane->normal[1];
			trace->plane.normal[2] = clipplane->normal[2];
			trace->plane.pad[0] = clipplane->pad[0];
			trace->plane.pad[1] = clipplane->pad[1];
			trace->plane.signbits = clipplane->signbits;
...
void CM_DecompressVis (byte *in, byte *out)
{
	...
	do
	{
		...
		{
			c = row - (out_p - out);
			Com_DPrintf ("^1warning: Vis decompression overrun\n");
		}
		while (c)
		{
			*out_p++ = 0;
