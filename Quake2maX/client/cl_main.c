...
void Cmd_ForwardToServer (void)
{
	...
	{
		Com_Printf ("Unknown command \"^i%s^r\"\n", cmd);
		return;
	}
...
//ZOID
	...
	if (precache_check >= CS_IMAGES && precache_check < CS_IMAGES+MAX_IMAGES) 
	{
		...
		while (precache_check < CS_IMAGES+MAX_IMAGES &&	cl.configstrings[precache_check][0]) 
		{
			...
			if (!CL_CheckOrDownloadImage(fn, "pcx"))
				return; // started a download
		}
	...
	if (precache_check > ENV_CNT && precache_check < TEXTURE_CNT) 
	{
		if (allow_download->value && allow_download_maps->value) 
		{
			while (precache_check < TEXTURE_CNT) 
			{
				...
				Com_sprintf(fn, sizeof(fn), "env/%s%s", 
	...
	if (precache_check == TEXTURE_CNT+1) 
	{
		...
		if (allow_download->value && allow_download_maps->value) {
			while (precache_tex < numtexinfo) {
				...
				sprintf(fn, "textures/%s", map_surfaces[precache_tex++].rname);
				if (!CL_CheckOrDownloadImage(fn, "wal"))
					return; // started a download
			}
		}
...
void CL_WriteConfiguration (char *configname);
void CL_WriteConfiguration_f (void)
{
	if (Cmd_Argc()<2)
		Com_Printf ("^2Error: bad file name");
	else
	{
...
//
	...
	cl_explosion = Cvar_Get ("cl_explosion", "0", CVAR_ARCHIVE);
	...
	cl_hudres = Cvar_Get ("cl_hudres", "640", CVAR_ARCHIVE);
	...
	skin = Cvar_Get ("skin", va("%s/%s", DEFAULTMODEL, DEFAULTSKIN), CVAR_USERINFO | CVAR_ARCHIVE);
...
// 	Cmd_AddCommand ("packet", CL_Packet_f); // this is dangerous to leave in
	...
	Cmd_AddCommand ("3dcamera", CL_ToggleCam_f);

	Cmd_AddCommand ("config", CL_WriteConfiguration_f);
...
void CL_WriteConfiguration (char *configname)
{
	...
	Com_sprintf (path, sizeof(path),"%s/%s.cfg",FS_Gamedir(), configname);
	f = fopen (path, "w");
	if (!f)
	{
	...
	fprintf (f, va("// generated by %s, do not modify\n", GAMENAME));
	Key_WriteBindings (f);
	fclose (f);
...
===============
*/
void CL_WriteUservarsConfiguration (void)
{
	FILE	*f;
	char	path[MAX_QPATH];
	...
	Com_sprintf (path, sizeof(path),"%s/uservars.cfg",FS_Gamedir());
	f = fopen (path, "w");
	if (!f)
	{
		Com_Printf ("Couldn't write uservars.cfg.\n");
		return;
	}
...
void CL_Frame (int msec)
{
	...
	if (!cl_timedemo->value)
	{
		...
		if (cl_maxfps->value>0 && extratime < 1000/cl_maxfps->value)
...
void CL_Shutdown(void)
{
	...
	CL_WriteConfiguration ("maxconfig"); 
