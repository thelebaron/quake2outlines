...
void InitLayouts () {
	char qwerty[94] = "`1234567890-=qwertyuiop[]asdfghjkl;'zxcvbnm,./\\~!@#$%^&*()_+QWERTYUIOP{}ASDFGHJKL:\"ZXCVBNM<>?|";
	int layout[94];
	FILE *f;
	int i,u;

	for(i=0; i<256; i++) normkey[i] = i;

	f = fopen ("keybinds.txt", "rb");
	if (!f)
		return;

	for(i=0; i<94; i++) {
		layout[i] = (int)fgetc(f);
	}
	fclose(f);
...
void Key_Event (int key, qboolean down, unsigned time)
{
	...
	if (!cls.consoleActive && cl.attractloop && cls.key_dest != key_menu &&
		!(key >= K_F1 && key <= K_F12) && cl.cinematictime > 0 && cls.realtime - cl.cinematictime > 1000)
		key = K_ESCAPE;
