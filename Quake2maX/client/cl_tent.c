...
void CL_Explosion_Particle (vec3_t org, float scale, float grow);
...
void copyEntity (entity_t *oldEnt, entity_t *newEnt)
{
	int i ;

	newEnt->alpha		= oldEnt->alpha;
	newEnt->backlerp	= oldEnt->backlerp;
	newEnt->flags		= oldEnt->flags;
	newEnt->frame		= oldEnt->frame;
	newEnt->lightstyle	= oldEnt->lightstyle;
	newEnt->model		= oldEnt->model;
	newEnt->oldframe	= oldEnt->oldframe;
	newEnt->skin		= oldEnt->skin;
	newEnt->skinnum		= oldEnt->skinnum;

	for (i=0;i<3;i++)
	{
		newEnt->angles[i]		= oldEnt->angles[i];
		newEnt->oldorigin[i]	= oldEnt->oldorigin[i];
		newEnt->origin[i]		= oldEnt->origin[i];
	}
}
...
{
	CL_Explosion_Particle (origin, 50, 25);
}

void CL_G_Explode_SP (vec3_t origin)
{
	CL_Explosion_Particle (origin, 75, 0);
}
...
void CL_ParseTEnt (void)
{
	...
	case TE_SHOTGUN:
		{
			...
			if (type != TE_SPARKS)
			{
				...
				if (!r_decals->value)
					re.AddStain(pos, 15, 50, 50, 50, 256, stain_subtract);
			}
			break;
		}		
	...
	case TE_BLASTER:			// blaster hitting wall
		...
		MSG_ReadDir (&net_message, dir);
		CL_BlasterParticles (pos, dir, 40);
		CL_ParticleBlasterDecal(pos, dir, 12.5);
...
//			CL_Radius_Explode_SP(pos, 1.25);
	...
	case TE_EXPLOSION1_BIG:	
		...
		if (modType("dday"))
		{
			CL_Explosion_Particle (pos, 250, 0);
			CL_ExplosionParticles (pos, 5);
		}
		else
		{
			CL_Explosion_Particle (pos, 100, 0);
	...
	case TE_EXPLOSION1_NP:						// PMM
		...
		CL_Explosion_Particle (pos, 50, 0);
	...
	case TE_ROCKET_EXPLOSION_WATER:
		...
		CL_ExplosionParticles (pos, 1);									// PMM
//		CL_Radius_Explode_SP(pos, 1);
